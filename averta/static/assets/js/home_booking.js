(function () {
    ...
    var root = document.getElementById('hero-booking');
    if (!root) return;
    ...
    function updatePickerLabel(kind) {
        ...
        // Desktop-da daha çox ad göstər, mobil-də qısa saxla
    ...
    function updateGuestsSummary() {
        ...
        var a = parseInt(aInp && aInp.value, 10); if (isNaN(a)) a = 1;
        var c = parseInt(cInp && cInp.value, 10); if (isNaN(c)) c = 0;
    ...
    function openGuestsModal() {
        ...
        requestAnimationFrame(function () { guestsModal.classList.add('is-open'); });
        if (guestsOpenBtn) guestsOpenBtn.classList.add('is-open');
        updateGuestsSummary();
    }
    ...
    function closeGuestsModal() {
        ...
        if (guestsOpenBtn) guestsOpenBtn.classList.remove('is-open');
    ...
    if (guestsOpenBtn) {
        guestsOpenBtn.addEventListener('click', function (e) {
            e.preventDefault();
            if (guestsModal && !guestsModal.hidden) closeGuestsModal();
            else openGuestsModal();
        });
    }
    ...
    document.addEventListener('click', function (e) {
        if (!guestsModal || guestsModal.hidden) return;
        if (root.contains(e.target) &&
            (e.target.closest('[data-guests-open]') || e.target.closest('[data-guests-modal]'))) return;
        closeGuestsModal();
    });

    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && guestsModal && !guestsModal.hidden) closeGuestsModal();
    });
