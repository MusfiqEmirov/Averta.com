(function () {
    ...
    var urls = Array.isArray(window.serviceGalleryUrls) ? window.serviceGalleryUrls.slice() : [];
    if (!urls.length) return;

    var root = document.querySelector('.aglb[data-sdp-lightbox]');
    if (!root) return;
    ...
    function onDocClick(e) {
        var card = e.target && e.target.closest ? e.target.closest('.sdp-gallery-card') : null;
        if (!card) return;
        e.preventDefault();
        var i = parseInt(card.getAttribute('data-gallery-index'), 10);
